...
for k in tqdm(range(0, num_pasos)):
    ...
    if num_vacantes[k] > int(0.85*num_max_vacantes):
        ...
        save_path = simulation_path + f'Figures/LLENADO_temperature_pp_set_{num_simulation}'
        RepresentateState(actual_state, simulation_path + f'Figures/LLENADO_configuration_pp_set_{num_simulation}.png')
    ...
    if Percolation.is_path(actual_state):
        ...
        try:
            ...
            RepresentateState(resistance_matrix, simulation_path +
                              f'Figures/Null_Resistance/NULL_resistance_matrix_pp_set_{num_simulation}.png')
...
np.savetxt(set_simulation_path + f'resultados_pp_set_{num_simulation}.csv',
...
np.savetxt(set_simulation_path + f'Resultados_sp_set_{num_simulation}.csv',
...
for k in tqdm(range(0, num_pasos)):
    ...
    if Percolation.is_path(actual_state):
        ...
        try:
            ...
            print("Null resistance matrix in ", filename)
            RepresentateState(resistance_matrix, simulation_path +
                              f'Figures/NULL_resistance_pp_reset_{num_simulation}.png')
            with open(filename, 'wb') as f:
                pickle.dump({"actual_state": ac, "resistance_matrix": resistance_matrix}, f)
    else:
        # Si no ha percolado uso la corriente de Poole-Frenkel
...
np.savetxt(reset_simulation_path +
           f'resultados_pp_reset_{num_simulation}.csv', data_pp_reset, header=header_files, delimiter=',')
...
for k in tqdm(range(0, num_pasos)):  # son num_pasos + 1 iteraciones
    ...
    if Percolation.is_path(actual_state):
        ...
        try:
            ...
            print("Null resistance matrix in ", filename)
            RepresentateState(resistance_matrix, simulation_path +
                              f'Figures/PR_resistance_matrix_{num_simulation}.png')
            with open(filename, 'wb') as f:
                pickle.dump({"actual_state": ac, "resistance_matrix": resistance_matrix}, f)
    else:
        # Si no ha percolado uso la corriente de Poole-Frenkel
...
np.savetxt(reset_simulation_path +
           f'resultados_sp_reset_{num_simulation}.csv', data_sp_reset, header=header_files, delimiter=',')

RepresentateState(actual_state, simulation_path + f'Figures/final_Configuration_sp_reset_{num_simulation}.png')
RepresentateState(oxygen_state, simulation_path +
                  f'Figures/final_Oxygen_sp_reset_{num_simulation}.png', color=(0.878, 0.227, 0.370))
# endregion
